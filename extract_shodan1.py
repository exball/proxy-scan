...
def extract_ip_port(input_file):
    ...
    results = []
    seen = set()  # untuk deduplikasi (ip, port)
    current_ip = None
        ...
        for i, line in enumerate(lines):
            ...
            if current_ip and " / tcp" in line:
                ...
                if port_match:
                    ...
                    pair = (current_ip, port)
                    if pair not in seen:     # cek duplikasi
                        results.append(pair)
                        seen.add(pair)
...
if __name__ == "__main__":
    try:
        ...
        print("💡 Silakan laporkan error ini jika terus terjadi.")#!/usr/bin/env python3
"""
Script Interaktif untuk Ekstraksi IP:Port dari File Shodan
Menampilkan pilihan file yang tersedia untuk dipilih
"""
...
def extract_ip_port(input_file):
    ...
    results = []
    seen = set()  # untuk deduplikasi (ip, port)
    dup_count = 0  # hitung berapa duplikasi yang ditemukan
    current_ip = None
    ...
    try:
        ...
        for i, line in enumerate(lines):
            ...
            if current_ip and " / tcp" in line:
                ...
                if port_match:
                    ...
                    pair = (current_ip, port)
                    if pair in seen:
                        dup_count += 1  # duplikasi ditemukan
                    else:
                        results.append(pair)
                        seen.add(pair)
        ...
        # Tampilkan jumlah duplikasi yang ditemukan (tanpa daftar IP:Port)
        print(f"🔁 Jumlah duplikasi IP:Port yang ditemukan: {dup_count}")
        return results
